...
abstract class IMessageRepo {
  ...
  Future<Either<Failure, MessageModel>> update(MessageId messageId ,{required MessageUpdateParam data});
...
class _Impl implements IMessageRepo {
  ...
  @override
  Future<Either<Failure,  MessageModel>> update(MessageId messageId ,{required MessageUpdateParam data}) async{
    return await errorHandler(() async {
    return await _ref.supabaseClient
    .from(MessageModel.table.tableName)
    .update(data.toJson())

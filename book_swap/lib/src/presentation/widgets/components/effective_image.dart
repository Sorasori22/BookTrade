...
enum ImageShape {
  /// Rectangular shape with optional borderRadius
  rectangle,
...
class EffectiveImage extends StatelessWidget {
  ...
  /// Default height to use when no height is specified and the widget is in a flex container
  ...
  /// Widget to display when imageObject is null
  ...
  Widget _buildImage(BuildContext context, Size widgetSize) {
    if (imageObject == null) {
      return fallbackWidget != null
          ? SizedBox.fromSize(size: widgetSize, child: fallbackWidget!(context))
          : SizedBox.fromSize(size: widgetSize, child: _buildPlaceholder(context));
    }

    final url = getUrl != null && imageObject is CompressibleImageObject
  ...
  Widget _buildOctoImage(
    ...
    } else if (_getBorderRadius != null) {
      return ClipRRect(borderRadius: _getBorderRadius!, child: child);
    }
  ...
  BorderRadiusGeometry? get _getBorderRadius {
    if (shape == ImageShape.circle) return null;
    return borderRadius == null ? null : BorderRadius.circular(borderRadius!);
  }
  ...
  Widget _buildPlaceholder(BuildContext? context) => Skeleton.leaf(
        child: DecoratedBox(
          decoration: BoxDecoration(
            color:
                context != null ? Theme.of(context).colorScheme.surface : context?.colors.secondary,
            borderRadius: shape == ImageShape.circle ? null : _getBorderRadius,
            shape: shape == ImageShape.circle ? BoxShape.circle : BoxShape.rectangle,
          ),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    } else if (shape == ImageShape.circle && width != null) {
      // For circle shape, use width for both dimensions if height is not specified
      final size = Size(width!, width!);
      imageWidget = _buildImage(context, size);
    } else if (shape == ImageShape.circle && height != null) {
      // For circle shape, use height for both dimensions if width is not specified
      final size = Size(height!, height!);
      imageWidget = _buildImage(context, size);
    } else {
      // Use LayoutBuilder to determine available space
      imageWidget = LayoutBuilder(
        builder: (context, constraints) {
          // Handle infinite height constraints (e.g., when in a Column)
          if (constraints.maxHeight == double.infinity) {
            double effectiveHeight = height ?? defaultHeight;
            double effectiveWidth = width ?? constraints.maxWidth;

            // For circle shape, ensure width and height are equal
            if (shape == ImageShape.circle) {
              if (width != null && height == null) {
                ...
                // If neither is specified for a circle, use the smaller dimension
                final minDimension =
                    effectiveWidth < effectiveHeight ? effectiveWidth : effectiveHeight;
                effectiveWidth = minDimension;
                effectiveHeight = minDimension;
              }
            }

            final size = Size(effectiveWidth, effectiveHeight);
            return SizedBox(
          ...
          // Normal case with finite constraints
    ...
    if (border) {
      return Container(
        decoration: BoxDecoration(
          ...
          borderRadius:
              shape == ImageShape.circle ? null : BorderRadius.circular(borderRadius ?? 4),
          shape: shape == ImageShape.circle ? BoxShape.circle : BoxShape.rectangle,
        ),
        child: imageWidget,
      );

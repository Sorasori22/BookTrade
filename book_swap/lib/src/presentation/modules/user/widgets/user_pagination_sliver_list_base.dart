...
class UserSliverListBaseConfig {
  const UserSliverListBaseConfig();
}
...
final _configProvider = Provider.autoDispose<UserSliverListBaseConfig>((ref) {
  throw UnimplementedError();
});
...
class UserSliverListBase extends ConsumerWidget {
  const UserSliverListBase({
    super.key,
    this.initialParam,
    this.config = const UserSliverListBaseConfig(),
  });

  final UserListParam? initialParam;
  final UserSliverListBaseConfig config;

...
add column if not exists related_trade_id bigint references public.trade_requests(id) on delete cascade;
...
$$
    ...
    begin
        insert into public.notifications (user_id, title, content, notification_type, payload, related_trade_id)
        values (p_user_id, p_title, p_content, p_notification_type, p_payload, p_related_trade_id)
...
begin
    ...
    perform public.create_notification(
        ...
        new.owner_id,
        v_book.title,
        format('Swap request from %s', coalesce(v_requester.fullname, v_requester.username)),
        to_jsonb(new),
        new.id
    );
...
begin
    ...
    perform public.create_notification(
        ...
        format('Your trade request on %s has been rejected\nReason: %s', v_book.title, coalesce(new.reject_reason, 'No reason provided')),
        to_jsonb(new),
        new.id
    );
